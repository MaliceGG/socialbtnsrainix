...
```
- *"isBtn"* это включение и выключение кнопки. Тоесть *"isBtn": 1* озночает что кнопка включено, а *"isBtn": 0* озночает что кнопка выключено.  
...
- *"link":* это озночает куда направляет сама кнопка при нажатий. Например *"link": **"httрs://yоutube.cоm"*** озночает что при нажатий кнопки перекидывает на ютуб.
